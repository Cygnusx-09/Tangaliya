...
type DotShape = "circle" | "bar";
function barRect(x: number, y: number, r: number) {
  const h = r * 1.5, w = h * 2;
...
const FINE_CELL = CELL_SIZE / GRID_SUBDIV;
...
function getFineKey(fc: number, fr: number) {
  return `f:${fc},${fr}`;
}
// The lattice step size a given snap mode places/moves dots on.
function snapSpacing(mode: SnapMode): number {
...
interface Layer { id: string; name: string; visible: boolean; dots: Map<string, Dot>; }
...
// selected right now.
interface UndoSnapshot { layerId: string; dots: Map<string, Dot>; }
interface SerializedLayer { id: string; name: string; visible: boolean; dots: Dot[]; }
...
function dotsArrayToMap(arr: unknown): Map<string, Dot> {
...
function flattenLayers(layers: Layer[]): Dot[] {
  const m = new Map<string, Dot>();
  for (const l of layers) if (l.visible) for (const d of l.dots.values()) m.set(d.key, d);
  return Array.from(m.values());
}

function sceneToLayers(scene: SceneFile | null): Layer[] {
...
function constrainAngle15(sx: number, sy: number, ex: number, ey: number): { x: number; y: number } {
...
const CURVE_SEG = 24; // tessellation segments per anchor span
function pathPolyline(anchors: { x: number; y: number }[], curved: boolean): { x: number; y: number }[] {
...
function pathLength(anchors: { x: number; y: number }[]): { segs: number[]; total: number } {
  const segs: number[] = [];
  let total = 0;
...
type SpacingShape = "even" | "ramp" | "taper" | "pulse";
interface SpacingOpts { shape: SpacingShape; spacing: number; amount: number; count: number; }

// ρ(t) ≥ ε for each shape. `a` ∈ [-1,1] is the intensity (sign flips
// direction for ramp/taper); `count` is the cluster count for pulse.
function densityAt(shape: SpacingShape, t: number, a: number, count: number): number {
  switch (shape) {
...
function ellipsePolyline(cx: number, cy: number, rx: number, ry: number, segments = 180): { x: number; y: number }[] {
...
type ShapeKind = "ellipse" | "rect" | "diamond" | "triangle" | "polygon";
...
function shapeVertices(kind: ShapeKind, cx: number, cy: number, rx: number, ry: number, sides: number): { x: number; y: number }[] {
  switch (kind) {
    case "rect":
...
function keyFromPosition(x: number, y: number, spacing: number = HALF_CELL): { key: string; x: number; y: number } {
